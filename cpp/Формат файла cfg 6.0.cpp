...
typedef struct TAGOSCDefMod
{
	...
	AE_TIME		time;			//время прихода осциллограммы абсолютное	
	...
	WORD		nsec;			//число десятков наносекунд прошедших с момента конца предидущей микросекунды
...
typedef struct TAGParDefMod		//для DDM-2 пока не используется
{
...
//для окна синхронного текстового просмотра CFG_SYNCVIEW 
//размер блока CFG_SYNCVIEW = sizeof ( RECT ) + sizeof ( char );
//RECT - положение окна
...
//////////////////////////////////////////////////
//конфигурационный блок CFG_POSTWNDS содержит информацию о заведенных и настроенных пользователем дополнительных окнах корреляционных зависимостей АЭ параметров
//размер блока CFG_POSTWNDS: 
...
//если все три оси окна задействованы (>=0), то окно 3-х мерное 
...
//если задействованы (>=0) только 2 оси, то окно 2-х мерное
UINT C2DView::QueryCfgSize ( void )
{
    UINT cfg_size;
    cfg_size = sizeof ( ScaleData ) * 2; // two cfg blocks for scales
...
//////////////////////////////////////////////////
//конфигурационный блок CFG_HISTWNDS содержит информацию о заведенных и настроенных пользователем дополнительных окнах гистограммных зависимостей АЭ параметров
//размер блока CFG_HISTWNDS: 
...
//если обе оси окна задействованы (>=0), то окно 3-х мерное 
UINT C3DView::QueryCfgSize ( void )
{
    UINT cfg_size;
    cfg_size = sizeof ( ScaleData ) * 3;	// 3-cfg blocks for scales
...
//если задействована (>=0) только 1 ось, то окно 2-х мерное 
UINT C2DView::QueryCfgSize ( void )
{
    UINT cfg_size;
    cfg_size = sizeof ( ScaleData ) * 2; // two cfg blocks for scales
...
typedef struct TAGNetIPInfo	//структура параметров настройки сетевого концентратора
{
