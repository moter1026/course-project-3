...
x1 = layers.Conv1D(1024, kernel_size=8, strides=4, padding='same')(x1)
x1 = layers.BatchNormalization()(x1)
x1 = layers.Activation('relu')(x1)
x1 = layers.MaxPooling1D(pool_size=4)(x1)
...
x2 = layers.Conv1D(1024, kernel_size=8, strides=4, padding='same')(x2)
x2 = layers.BatchNormalization()(x2)
x2 = layers.Activation('relu')(x2)
...
# Compile model
model = tf.keras.Model(inputs=[input_signal_keras, input_spectrum_keras, input_dB_keras],
                      outputs=output)
...
callbacks = [
    ...
    # Сохраняем модель после каждой эпохи, если она лучше предыдущей
...
# Сохранение модели
model.save(f"model№3_24.12.26 description_24.12.20 ОСЦ ДЛЯ МАТВЕЯ ровно_copy_withEarlyStop_{epochs}-epochs_{batch_size}-batchsize.keras")
